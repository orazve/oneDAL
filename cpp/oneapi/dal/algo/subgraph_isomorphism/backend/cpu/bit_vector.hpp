...
public:
    ...
    static std::uint8_t get_bit_set_table(std::int64_t i) {
        ONEDAL_ASSERT(i < 256);
...
void and_equal(std::uint8_t* vec,
    ...
    for (std::int64_t i = 0; i < list_size; i++) {
        ...
        counter += bit_vector<Cpu>::get_bit_set_table(vec[bit_vector<Cpu>::byte(bit_index[i])] &
...
bit_vector<Cpu>& bit_vector<Cpu>::and_equal(const std::int64_t* bit_index,
    ...
    for (std::int64_t i = 0; i < list_size; i++) {
        tmp_array[counter] = bit_index[i];
        counter += get_bit_set_table(vector[byte(bit_index[i])] & bit(bit_index[i]));
    }
...
std::int64_t bit_vector<Cpu>::get_bit_index(const std::int64_t vector_size,
    ...
    for (std::int64_t i = 0; i < nbyte; i++)
        result += bit_vector<Cpu>::get_bit_set_table(vector[i]);
...
std::int64_t bit_vector<Cpu>::popcount(const std::int64_t vector_size, const std::uint8_t* vector) {
    ...
    std::int64_t result = 0;
    ___PR___(vector_size);
    for (std::int64_t i = 0; i < vector_size; i++) {
        // ___PR___(i);
        // ___PR___(((int)vector[i]));
        // int val = bit_vector<Cpu>::get_bit_set_table(vector[i]);
        // ___PR___(val);
        result += bit_vector<Cpu>::get_bit_set_table(vector[i]);
    }
