...
class GetterParamsForestPredict {
public:
  using nTrees       = size_t;
  ...
  GetterParamsForestPredict(const Params& params) : params(params) {}

  static DictionaryParams<Params> get_params() {
    return {
      { "Susy:4.5M;nT:50;D:5",
        Params(DatasetName("SUSY"), TableType(SyclHomogen, FPType), nTrees(50), maxTreeDepth(5)) },
      { "Mnist:60K;nT:100;D:10",
        Params(DatasetName("mnist"),
               ...
               maxTreeDepth(10)) },
      { "Mnist:60K;nT:40;D:0",
        Params(DatasetName("mnist"), TableType(SyclHomogen, FPType), nTrees(40), maxTreeDepth(0)) },
      { "Hepmass:150K;nT:80;D:15",
        Params(DatasetName("hepmass_small_classifier"),
               TableType(SyclHomogen, FPType),
               nTrees(80),
               maxTreeDepth(15)) },
      { "Hepmass:150K;nT:200;D:5",
        Params(DatasetName("hepmass_small_classifier"),
               TableType(SyclHomogen, FPType),
               nTrees(200),
               maxTreeDepth(5)) }
